...
public class SoloMixer extends AbstractSessionMixerMode {
    ...
    private BooleanSyncWrapper[] mIsSoloed = new BooleanSyncWrapper[8];
    private HardwareActionBindable[] mSoloAction = new HardwareActionBindable[8];
    private static final ColorTag mOnColor = new ColorTag(0xf3, 0xee, 0x61);
    private static final ColorTag mOffColor = new ColorTag(0xb3, 0xa1, 0x61);

    public SoloMixer(ModeMachine machine, AtomicReference<Mode> mixerMode, ControllerHost host, Transport transport,
                     LaunchpadXSurface lSurf, HardwareSurface surf, CursorTrack _track, TrackBank bank) {
        super(machine, mixerMode, host, transport, lSurf, surf, _track, bank, Mode.MIXER_SOLO);

        for(int i = 0; i < 8; i++) {
            Track track = bank.getItemAt(i);
            mTrackExists[i] = new BooleanSyncWrapper(track.exists(), surf, host);
            mIsSoloed[i] = new BooleanSyncWrapper(track.solo(), surf, host);
    ...
    public List<HardwareBinding> onBind(LaunchpadXSurface surface) {
        ...
        bindMixerModeIndicator(surface, list, 6, new ColorTag(0xf3, 0xee, 0x61));
        ...
        for(int i = 0; i < 8; i++) {
            ...
            pad.light().state().setValue(new ToggleLight(pad.id(), mTrackExists[i], mIsSoloed[i], mOnColor, mOffColor));
        }
